...
class RectSolidIndicator(context: Context, @ColorInt val color: Int = context.resources.getColor(R.color.colorAccent)) : BaseIndicator(context) {
    ...
    override fun onDraw(canvas: Canvas?, parent: ViewGroup?, selectedPos: Int, selectedOffset: Float) {
        ...
        if (showIndicator) {
            if (childCount > 0) {
                ...
                canvas!!.drawRect(RectF((middle - indicatorWidth / 2).toFloat(), 0F,
                        (middle + indicatorWidth / 2).toFloat(), height.toFloat()), mSelectedIndicatorPaint)
            }
        }
    }
